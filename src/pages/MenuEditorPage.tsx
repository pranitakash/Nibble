...
const MenuEditorPage: React.FC = () => {
    const navItems = [
        { icon: 'menu_book', label: 'Menu Editor', href: '/restaurant/menu-editor', active: true },
    ];
    ...
    const openEdit = (item: typeof initialItems[0]) => { setForm({ ...item, price: String(item.price) }); setEditingId(item.id); setShowForm(true); };
    ...
    return (
        <div className="bg-background-light text-accent-dark antialiased min-h-screen font-display flex flex-col">
            <DashboardNavbar items={navItems} statusSection={storeStatus} rightSection={actionButtons} profileName="Big Kahuna Burger" />
                        ...
                        {displayed.map((item, idx) => (
                            <motion.div key={item.id}
                                ...
                                <div className="flex gap-2 mt-4 flex-wrap">
                                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => openEdit(item)}
                                    ...
                                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => toggleAvailable(item.id)}
                                    ...
                                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => deleteItem(item.id)}
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
                            ...
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Crispy Chicken Burger"
                                ...
                                <div>
                                    ...
                                    <textarea rows={3} value={form.desc} onChange={e => setForm(p => ({ ...p, desc: e.target.value }))} placeholder="Describe the dish…"
                            ...
                            <div className="flex gap-4 mt-8">
                                ...
                                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => setShowForm(false)}
